...
public class LibraryManagementSystem extends JFrame {
        ...
        updateBtn.addActionListener(e -> {
            ...
            if (selectedRow != -1) {
                ...
                if (!title.isEmpty() && !author.isEmpty() && !isbn.isEmpty() && !category.isEmpty() && !qtyStr.isEmpty()) {
                    try {
                        int qty = Integer.parseInt(qtyStr);
                        books.get(selectedRow).setTitle(title);
                        books.get(selectedRow).setAuthor(author);
                        books.get(selectedRow).setIsbn(isbn);
                        books.get(selectedRow).setCategory(category);
                        books.get(selectedRow).setQuantity(qty);
    ...
    private JPanel createSearchPanel() {
        ...
        searchBtn.addActionListener(e -> {
            ...
            for (Book book : books) {
                boolean match = false;
                if (searchType.equals("Title") && book.getTitle().toLowerCase().contains(searchValue)) {
                    match = true;
                } else if (searchType.equals("Author") && book.getAuthor().toLowerCase().contains(searchValue)) {
                    match = true;
                } else if (searchType.equals("Category") && book.getCategory().toLowerCase().contains(searchValue)) {
                    match = true;
                }

                if (match) {
                    searchTableModel.addRow(new Object[]{book.getTitle(), book.getAuthor(), book.getIsbn(), book.getCategory(), book.getQuantity()});
                }
            }
    ...
    private JPanel createIssueReturnPanel() {
        ...
        issueBtn.addActionListener(e -> {
            ...
            try {
                ...
                bookTableModel.setValueAt(book.getQuantity(), bookIdx, 4);
                
                studentTableModel.setValueAt((int) studentTableModel.getValueAt(studentIdx, 3) + 1, studentIdx, 3);
                
                issuedTableModel.addRow(new Object[]{student.getName(), book.getTitle(), issueDate, dueDate, "Active"});
            ...
            if (selectedRow != -1) {
                ...
                for (Student student : students) {
                    if (student.getName().equals(studentName)) {
                        for (IssuedBook issuedBook : student.getIssuedBooks()) {
                            if (issuedBook.getBook().getTitle().equals(bookTitle) && issuedBook.getReturnDate() == null) {
                                issuedBook.setReturnDate(LocalDate.now());
                                issuedBook.getStatus();
                                
                                issuedBook.getBook().setQuantity(issuedBook.getBook().getQuantity() + 1);
                                for (int i = 0; i < books.size(); i++) {
                                    if (books.get(i).getTitle().equals(bookTitle)) {
                                        bookTableModel.setValueAt(issuedBook.getBook().getQuantity(), i, 4);
                                        break;
                                    }
                                }
                                
                                String fineMsg = issuedBook.getFine() > 0 ? "\nFine: Rs" + issuedBook.getFine() : "";
        ...
        studentCombo.addActionListener(e -> {
            ...
            for (Book book : books) {
                if (book.getQuantity() > 0) {
                    bookCombo.addItem(book.getTitle());
                }
            }
        });
        ...
        if (!students.isEmpty()) {
            for (Student student : students) {
                studentCombo.addItem(student.getName());
            }
        }
